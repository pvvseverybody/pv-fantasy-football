...
export async function getSystemReadiness({environment=process.env,reader=readSheetRange}={}){
  ...
  const releaseMode=evaluateReleaseMode(environment);const betaAcceptance=readBetaAcceptance(environment);const deploymentSafety=evaluateDeploymentSafety(environment);
  if(configuration.groups.workbook!=='CONFIGURED')return evaluateSystemReadiness({configuration,connectivity:{status:'UNAVAILABLE',schema:{status:'UNKNOWN'}},preflight:{status:'BLOCKED'},gameReadiness:null,releaseMode,betaAcceptance,deploymentSafety});
  ...
  if(!tablesResult.tables)return evaluateSystemReadiness({configuration,connectivity:{status:'UNAVAILABLE',schema:connectivity.schema},preflight:{status:'BLOCKED'},gameReadiness:null,releaseMode,betaAcceptance,deploymentSafety});
  ...
  return evaluateSystemReadiness({configuration,connectivity,preflight,gameReadiness,releaseMode,betaAcceptance,deploymentSafety});
}
