...
export async function getSystemReadiness({environment=process.env,reader=readSheetRange,betaApproved=false}={}){
  const configuration=evaluateProductionConfig(environment);
  if(configuration.groups.workbook!=='CONFIGURED')return evaluateSystemReadiness({configuration,connectivity:{status:'UNAVAILABLE',schema:{status:'UNKNOWN'}},preflight:{status:'BLOCKED'},gameReadiness:null,betaApproved});
  ...
  if(!tablesResult.tables)return evaluateSystemReadiness({configuration,connectivity:{status:'UNAVAILABLE',schema:connectivity.schema},preflight:{status:'BLOCKED'},gameReadiness:null,betaApproved});
  ...
  return evaluateSystemReadiness({configuration,connectivity,preflight,gameReadiness,betaApproved});
}
